    ...
    <div className="space-y-6 pt-4 animate-fadeIn">
      <div className="flex justify-between items-center bg-zinc-950/40 p-4 border border-zinc-800/60 rounded-2xl">
        <div>
          <h3 className="text-sm font-mono font-semibold text-emerald-400 uppercase tracking-wider">
            Project Portfolio Trackers
          </h3>
          <p className="text-xs text-zinc-500 font-light mt-0.5">
            Control completion matrices, lifecycle states, and stack sliders.
          </p>
        </div>
        <button
          type="button"
          onClick={() =>
            updateDraft((d) => {
              d.projects.push({
                ...
                description:
                  'In-progress system architecture description summary.',
                stack: [
                  { name: 'React', level: 50 },
      ...
      <div className="space-y-6 max-h-137.5 overflow-y-auto pr-2 custom-scrollbar">
        {draft.projects.map((project, index) => {
          // Safe normalization for mixed formats
          const stackItems = (project.stack || []).map((s: any) => {
            if (typeof s === 'string') {
              const [name = 'Tech', val = '0'] = s.split(':');
              return {
                name: name.trim(),
                value: parseInt(val.trim() || '0', 10),
              };
            } else if (s && typeof s === 'object') {
              return {
                name: typeof s.name === 'string' ? s.name.trim() : 'Tech',
                value:
            ...
            return { name: 'Tech', value: 0 };
          });

          const totalPercentage =
            stackItems.length > 0
              ? Math.round(
                  stackItems.reduce((acc, item) => acc + item.value, 0) /
                    stackItems.length
                )
              : 0;
          ...
          return (
            <div
              key={project.id}
              className="p-5 bg-zinc-950/80 border border-zinc-800/80 rounded-2xl relative space-y-6 group"
            >
              <button
                type="button"
                onClick={() =>
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateDraft((d) => {
                        d.projects[index].githubUrl = e.target.value;
                      })
                    }
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateDraft((d) => {
                        d.projects[index].liveUrl = e.target.value;
                      })
                    }
                    className="w-full bg-zinc-900 border border-zinc-800/80 p-2 rounded-xl text-sm outline-none text-zinc-300 focus:border-emerald-500/30"
                  >
              ...
              <div className="grid sm:grid-cols-3 gap-4 pt-2 border-t border-zinc-900">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateDraft((d) => {
                        d.projects[index].deploymentUrl = e.target.value;
                      })
                    }
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateDraft((d) => {
                        d.projects[index].sourceCodeUrl = e.target.value;
                      })
                    }
                    className="w-full bg-zinc-900 border border-zinc-800/80 p-2 rounded-xl text-xs font-mono outline-none text-zinc-300 focus:border-emerald-500/30"
                  />
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-mono text-zinc-500">
                    Framework Infrastructure Array (Comma Separated)
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      updateDraft((d) => {
                        d.projects[index].frameworksArray = e.target.value
                          .split(',')
                          .map((s) => s.trim())
                          .filter(Boolean);
                      })
              ...
              {/* Grid 2: Description & Completion */}
              <div className="grid sm:grid-cols-4 gap-4 items-start">
                <div className="sm:col-span-3 space-y-1">
                  <label className="text-[11px] font-mono text-zinc-500">
                    Repository Description Narrative Summary
                  </label>
                  <textarea
                    rows={2}
                ...
                <div className="pt-6 flex h-full items-center">
                  <label className="flex items-center gap-2 cursor-pointer select-none text-[11px] font-mono text-zinc-400 bg-zinc-900/50 p-3 rounded-xl border border-zinc-800 w-full hover:border-zinc-700 transition-colors">
                    <input
                      ...
                      onChange={(e) =>
                        updateDraft((d) => {
                          d.projects[index].featured = e.target.checked;
                        })
                      }
              ...
              <div className="bg-zinc-900/40 p-4 border border-zinc-800/60 rounded-xl space-y-4">
                <div className="flex justify-between items-center pb-2 border-b border-zinc-800/40">
                  <span className="text-xs font-mono font-medium text-zinc-400">
                    Technical Skill Matrix Sliders
                  </span>
                  <button
                    type="button"
                    onClick={() =>
                  ...
                  <div className="pt-4 border-t border-zinc-800/40 space-y-2">
                    <div className="flex justify-between text-[11px] font-mono">
                      ...
                      <span className="text-emerald-400 font-semibold">
                        Total Stability Index: {totalPercentage}%
                      </span>
                    </div>
