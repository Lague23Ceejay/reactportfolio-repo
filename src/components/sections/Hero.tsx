...
export function Hero() {
  ...
  return (
    <section className="flex flex-col-reverse md:flex-row items-center justify-between gap-12 min-h-[65vh] py-12">
      {/* LEFT COLUMN: text — staggered left reveals */}
      <div className="flex-1 flex flex-col items-start justify-center cursor-target">
        <RevealGroup direction="left" baseDelay={0} step={0.06}>
      ...
      {/* RIGHT COLUMN: profile image cloud — single reveal from down */}
      <div className="flex items-center justify-center relative w-72 h-72 md:w-80 md:h-80 select-none group">
        ...
        <ScrollReveal direction="down" delay={0.04}>
          <div
          ...
          >
            {profileImage ? (
              <PixelImageTransition
                ...
                firstContent={
                  <img src={profileImage} alt="Primary Profile Avatar" className="hidden" />
                }
                secondContent={
                  data?.hero?.profileImageSecondary ? (
                    <img src={data.hero.profileImageSecondary} alt="Secondary Profile Avatar" className="hidden" />
                  ) : undefined
                }
              />
            ) : (
